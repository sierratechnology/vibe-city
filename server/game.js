...
export class Game {
 constructor(world=makeWorld()){this.world=world;world.containers??={};world.nextDrop??=1;world.creatures??=spawnCreatures(world.seed);for(const p of Object.values(world.players)){p.inventory.meat??=0;p.inventory.ration??=0;p.flashlightOwned??=false;p.flashlightOn??=false;}this.inputs=new Map();this.online=new Set();this.cooldowns=new Map();}
 join(id,name){let p=this.world.players[id];if(!p)p=this.world.players[id]={...makePlayer(id,name),flashlightOwned:false,flashlightOn:false};else p.name=name;p.inventory.meat??=0;p.inventory.ration??=0;this.online.add(id);this.inputs.set(id,{x:0,z:0});return p;}
 leave(id){this.online.delete(id);this.inputs.delete(id);this.cooldowns.delete(id);}
 input(id,m){if(!this.online.has(id))return;this.inputs.set(id,{x:Math.max(-1,Math.min(1,Number(m.x)||0)),z:Math.max(-1,Math.min(1,Number(m.z)||0)),yaw:Number.isFinite(m.yaw)?m.yaw:0,sprint:m.sprint===true,aimYaw:Number.isFinite(m.aimYaw)?m.aimYaw:0});}
 tick(dt){
