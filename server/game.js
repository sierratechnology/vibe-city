...
export class Game {
 constructor(world=makeWorld()){this.world=world;initExpedition(world);world.containers??={};world.nextDrop??=1;world.creatures??=spawnCreatures(world.seed);for(const p of Object.values(world.players)){p.inventory.meat??=0;p.inventory.ration??=0;p.flashlightOwned??=false;p.flashlightOn??=false;}this.inputs=new Map();this.online=new Set();this.cooldowns=new Map();}
 join(id,name){let p=this.world.players[id];if(!p)p=this.world.players[id]={...makePlayer(id,name),flashlightOwned:false,flashlightOn:false};else p.name=name;p.inventory.meat??=0;p.inventory.ration??=0;initPlayer(p);this.online.add(id);this.inputs.set(id,{x:0,z:0});return p;}
 leave(id){this.online.delete(id);this.inputs.delete(id);this.cooldowns.delete(id);}
 input(id,m){if(!this.online.has(id))return;this.inputs.set(id,{x:Math.max(-1,Math.min(1,Number(m.x)||0)),z:Math.max(-1,Math.min(1,Number(m.z)||0)),yaw:Number.isFinite(m.yaw)?m.yaw:0,sprint:m.sprint===true,aimYaw:Number.isFinite(m.aimYaw)?m.aimYaw:0,...([m.vx,m.vy,m.vz].every(Number.isFinite)?{vx:Math.max(-1,Math.min(1,m.vx)),vy:Math.max(-1,Math.min(1,m.vy)),vz:Math.max(-1,Math.min(1,m.vz))}:{})});}
 tick(dt){
 ...
 snapshot(viewer=null){const p=this.world.players[viewer];const resources=p?resourcesFor(this.world,p):this.world.resources;return{settings:this.world.settings,circumference:CIRCUMFERENCE,monuments:MONUMENTS,vehicles:this.world.vehicles.map(v=>({...v})),locks:Object.fromEntries(Object.keys(this.world.locks).map(key=>[key,{locked:!p||!canAccess(this.world,p,key)}])),seed:this.world.seed,time:this.world.time,structures:this.world.structures,containers:Object.fromEntries(Object.entries(this.world.containers).filter(([key])=>!this.world.locks[key]||(p&&canAccess(this.world,p,key)))),creatures:this.world.creatures.filter(c=>activeCreature(c,this.world.time,this.world.settings)),resources,playerCount:this.online.size,players:[...this.online].map(id=>{const q=this.world.players[id];if(viewer&&id!==viewer)return{id:q.id,name:q.name,x:q.x,z:q.z,yaw:q.yaw,aimYaw:q.aimYaw,health:q.health,charge:q.charge,flashlightOn:q.flashlightOn,vehicle:q.vehicle,suit:q.suit};return{...q,room:roomStatus(this.world,q)};})};}
}
