...
function dismantleGrants(world,structure){const grants=structure.dismantleGrants;if(grants===undefined)return[];if(!Array.isArray(grants)||grants.length>DISMANTLE_GRANT_LIMIT||grants.length>Object.keys(world.players).length||new Set(grants).size!==grants.length||grants.some(target=>typeof target!=='string'||target.length>128||FORBIDDEN_PLAYER_IDS.has(target)||target===structure.owner||!Object.hasOwn(world.players,target)))return null;return grants;}
function nextWorkbenchSequenceFor(bench){if(Number.isSafeInteger(bench.workbenchNextSequence)&&bench.workbenchNextSequence>=0)return bench.workbenchNextSequence;if(bench.workbenchNextSequence===undefined&&(!bench.workbenchQueue||bench.workbenchQueue.length===0)&&(!bench.workbenchRequests||bench.workbenchRequests.length===0))return 0;return null;}
function workbenchQueueFor(world,bench){const queue=bench.workbenchQueue;if(queue===undefined)return[];if(!Array.isArray(queue)||queue.length>3)return null;if(!queue.length)return queue;const nextSequence=nextWorkbenchSequenceFor(bench);if(nextSequence===null||nextSequence<queue.length)return null;const ids=new Set(),firstSequence=nextSequence-queue.length;for(let index=0;index<queue.length;index++){const job=queue[index],active=index===0,sequence=firstSequence+index;if(!job||typeof job!=='object'||Array.isArray(job)||typeof job.id!=='string'||!/^[A-Za-z0-9_-]{1,64}$/.test(job.id)||(!((sequence===0&&job.id!=='workbench-0')||job.id===`workbench-${sequence}`))||ids.has(job.id)||job.owner!==bench.owner||!Object.hasOwn(world.players,job.owner)||job.recipe!=='ration'||(active?!['in-progress','blocked-output'].includes(job.state):job.state!=='queued'))return null;const keys=Object.keys(job).sort().join(',');if(active){const elapsed=world.time-job.startedAt;if(keys!=='id,owner,recipe,startedAt,state'||!Number.isFinite(job.startedAt)||job.startedAt<0||job.startedAt>world.time||(job.state==='blocked-output'&&elapsed<WORKBENCH_DURATION))return null;}else if(keys!=='id,owner,recipe,state')return null;ids.add(job.id);}return queue;}
export class Game {
 ...
 for(const bench of this.world.structures.filter(s=>s.type==='workbench')){const queue=workbenchQueueFor(this.world,bench);if(!queue?.length)continue;const job=queue[0],owner=this.world.players[job.owner];if(job.state==='in-progress'&&this.world.time-job.startedAt>=WORKBENCH_DURATION)job.state='blocked-output';if(job.state==='blocked-output'&&owner&&freeSpace(owner.inventory)>=1){owner.inventory.ration++;queue.shift();if(queue[0]){queue[0].state='in-progress';queue[0].startedAt=this.world.time;}}}
 ...
 if(m.type==='workbench'){
 const keys=Object.keys(m).sort(),legacyRequest=keys.length===4&&keys.join(',')==='id,recipe,requestId,type';if((!legacyRequest&&(keys.length!==4||keys.join(',')!=='id,recipe,sequence,type'))||m.recipe!=='ration'||typeof m.id!=='string'||(legacyRequest?(typeof m.requestId!=='string'||!/^[A-Za-z0-9_-]{1,64}$/.test(m.requestId)):(!Number.isSafeInteger(m.sequence)||m.sequence<0||m.sequence>=Number.MAX_SAFE_INTEGER)))return fail('Invalid workbench request.');
 const bench=this.world.structures.find(s=>s.id===m.id&&s.type==='workbench');if(!bench||dist(p,bench)>3)return fail('Move within 3 m of your workbench.');if(bench.owner!==id)return fail('Only the workbench builder may use it.');
 const queue=workbenchQueueFor(this.world,bench);if(queue===null)return fail('Invalid workbench queue state.');const nextSequence=nextWorkbenchSequenceFor(bench),sequence=legacyRequest?0:m.sequence;if(nextSequence===null)return fail('Invalid workbench replay state.');if(sequence!==nextSequence)return fail('This workbench sequence is stale or skipped.');if(queue.length>=3)return fail('Workbench queue full.');if(itemCount(p.inventory)-itemCount(RECIPES.ration.cost)+1>60)return fail('Make room for the finished field meal.');if(!canAfford(p,'ration'))return fail('Not enough ingredients.');
 ...
 if(m.type==='craft'){
 if(m.recipe==='ration')return fail('Prepare field meals at a nearby workbench.');if(!['cutter','flashlight','rifle','repair'].includes(m.recipe))return fail('Unknown recipe.');
 ...
 const s=this.world.structures.find(s=>s.id===m.id);if(!s||s.owner!==id)return fail('Only its builder can manage dismantling access.');
 if(typeof m.target!=='string'||m.target.length>128||FORBIDDEN_PLAYER_IDS.has(m.target)||m.target===id||!Object.hasOwn(this.world.players,m.target))return fail('Choose a saved fellow explorer.');
 const grants=dismantleGrants(this.world,s);if(!grants)return fail('Dismantling access is invalid.');
 if(m.allow===false){if(!grants.includes(m.target))return fail('That explorer does not have dismantling access.');const remaining=grants.filter(target=>target!==m.target);if(remaining.length)s.dismantleGrants=remaining;else delete s.dismantleGrants;return{ok:true,message:`${this.world.players[m.target].name} no longer has dismantling access.`};}
 if(m.allow!==true)return fail('Choose grant or revoke.');if(grants.includes(m.target))return fail('That explorer already has dismantling access.');if(grants.length>=DISMANTLE_GRANT_LIMIT)return fail('This piece has reached its dismantling access limit.');s.dismantleGrants=[...grants,m.target];return{ok:true,message:`${this.world.players[m.target].name} may dismantle this piece.`};
 }
 if(m.type==='dismantle'){
 const i=this.world.structures.findIndex(s=>s.id===m.id),s=this.world.structures[i];if(!s||dist(p,s)>6)return fail('Move closer to the piece.');if(s.owner!==id&&!dismantleGrants(this.world,s)?.includes(id))return fail('Only its builder or an authorized collaborator can dismantle this piece.');
 ...
 snapshot(viewer=null){const p=this.world.players[viewer],resources=p?resourcesFor(this.world,p):this.world.resources,structures=this.world.structures.map(s=>{const{dismantleGrants:storedGrants,workbenchQueue,workbenchRequests,workbenchNextSequence,...visible}=s,grants=dismantleGrants(this.world,s);if(viewer===s.owner){visible.canDismantle=true;visible.dismantleGrantedTo=grants?[...grants]:[];if(s.type==='workbench'){const queue=workbenchQueueFor(this.world,s),nextSequence=nextWorkbenchSequenceFor(s);if(queue!==null&&nextSequence!==null){visible.cycleUntil=nextSequence;if(queue.length){const job=queue[0];visible.water=queue.length;visible.power=job.state==='blocked-output'?100:Math.max(0,Math.min(100,(this.world.time-job.startedAt)/WORKBENCH_DURATION*100));visible.readyAt=job.startedAt+WORKBENCH_DURATION;}}}}else if(grants?.includes(viewer))visible.canDismantle=true;return visible;});return{planet:this.world.planet,settings:this.world.settings,circumference:CIRCUMFERENCE,monuments:MONUMENTS,vehicles:this.world.vehicles.map(v=>({...v})),locks:Object.fromEntries(Object.keys(this.world.locks).map(key=>[key,{locked:!p||!canAccess(this.world,p,key)}])),seed:this.world.seed,time:this.world.time,structures,containers:Object.fromEntries(Object.entries(this.world.containers).filter(([key])=>!this.world.locks[key]||(p&&canAccess(this.world,p,key)))),creatures:this.world.creatures.filter(c=>activeCreature(c,this.world.time,this.world.settings)),resources,playerCount:this.online.size,players:[...this.online].map(id=>{const q=this.world.players[id];if(viewer&&id!==viewer)return{id:q.id,name:q.name,x:q.x,z:q.z,yaw:q.yaw,aimYaw:Number.isFinite(q.aimYaw)?q.aimYaw:q.yaw,health:q.health,charge:q.charge,flashlightOn:q.flashlightOn,vehicle:q.vehicle,suit:q.suit};return{...q,room:roomStatus(this.world,q)};})};}
}
