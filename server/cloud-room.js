...
export function advance(room,now=Date.now()){
 ...
 const game=new Game(room.world);game.online=new Set(Object.keys(room.leases));game.inputs=new Map(Object.entries(room.inputs).map(([id,v])=>[id,now-v.at<500?v:{x:0,z:0}]));
...
export function frameRoom(room,clients,actions,now=Date.now()){
 ...
 finish(room,game);return{results,state:game.snapshot()};
}
