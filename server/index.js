...
export function startServer({port=Number(process.env.PORT||4173),host=process.env.HOST||'0.0.0.0',saveFile=process.env.SAVE_FILE||path.join(root,'data/world.json'),seed=Number(process.env.SEED||7319)}={}){
 ...
 let backupDay=new Date().toISOString().slice(0,10);let ticks=0;const tick=setInterval(()=>{game.tick(.05);if(++ticks%2===0){const state=game.snapshot();for(const [playerId,ws] of sockets)send(ws,{type:'state',state,inputAck:game.inputAck(playerId),saveError,lastSaved});}if(ticks%100===0){save();const day=new Date().toISOString().slice(0,10);if(day!==backupDay){try{const dir=path.join(path.dirname(saveFile),'daily');saveWorld(path.join(dir,day+'.json'),game.world);backupDay=day;for(const file of fs.readdirSync(dir).filter(f=>/^\d{4}-\d{2}-\d{2}\.json$/.test(f)).sort().slice(0,-8))fs.unlinkSync(path.join(dir,file));}catch(e){console.error('Daily backup failed:',e.message);}}}},50);
