...
- One buildable owner-operated Workbench and its bounded Field meal queue are implemented: one active job, two pending jobs, five seconds of server-running game time, exact-once delivery, blocked-output preservation, and no offline catch-up. Shared cargo lockers, item transfers, PIN protection, and owner-managed full-transfer grants for individual saved collaborators are implemented. Broader recipes, shared workbench permissions, queue controls, storage roles, groups, organizations, offline invitations and automation remain planned.
