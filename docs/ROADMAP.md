...
- A first bounded sustainable-farming feedback slice is implemented for the existing Hydroponic bed: nearby explorers see an accepted server-time growth countdown, Harvest appears only when the accepted due is reached, and the existing water + fiber planting and fiber + food harvest path remains server-authoritative. This is not complete farming, offline growth, automation, crop choice, ownership policy or physical-device verification; broader tools, equipment progression, sustainable gathering/farming and optional threats remain planned.
