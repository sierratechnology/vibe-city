...
- One buildable owner-operated Workbench and its bounded Field meal queue are implemented: one active job, two pending jobs, five seconds of server-running game time, exact-once delivery, blocked-output preservation, and no offline catch-up. Broader recipes, shared workbench permissions, queue controls and storage permissions remain planned. Shared cargo lockers and item transfers are implemented.
