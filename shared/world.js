...
export function shape(piece,seed){
 ...
 if(['wall','doorway','perimeter','campGate','door','airlock','railing'].includes(piece.type)){dx=[0,1.5,0,-1.5][r];dz=[-1.5,0,1.5,0][r];dy=piece.type==='railing'?.72:1.55;w=r%2?.18:3;d=r%2?3:.18;h=piece.type==='railing'?1.1:2.6;}
...
export function placementError(world,p,piece,people=[]){
 ...
 if(same.some(s=>s.type===piece.type&&(!['wall','doorway','perimeter','campGate','door','airlock','stairs','railing','lamp'].includes(s.type)||s.rotation===piece.rotation))&&piece.type!=='stairs')return 'This slot is occupied.';
