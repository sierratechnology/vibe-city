...
const DECK_GATE_EDGE_TYPES=new Set(['deckGate','wall','doorway','door','airlock','perimeter','campGate','stairs','railing']);
...
export function shape(piece,seed){
 ...
 if(['wall','doorway','perimeter','campGate','door','airlock','railing','deckGate'].includes(piece.type)){dx=[0,1.5,0,-1.5][r];dz=[-1.5,0,1.5,0][r];dy=piece.type==='railing'?.72:1.55;w=r%2?.18:3;d=r%2?3:.18;h=piece.type==='railing'?1.1:2.6;}
...
export function placementError(world,p,piece,people=[]){
 if(!['floor','wall','doorway','roof','angledCanopy','stairs','railing','deckGate','heater','perimeter','campGate','lamp','cargo','workbench','door','airlock','lifeSupport','iceProcessor','garden','bed'].includes(piece.type))return 'Choose a construction piece.';
 ...
 if(same.some(s=>s.type===piece.type&&(!['wall','doorway','perimeter','campGate','door','airlock','stairs','railing','deckGate','lamp'].includes(s.type)||s.rotation===piece.rotation))&&piece.type!=='stairs')return 'This slot is occupied.';
 ...
 if(DECK_GATE_EDGE_TYPES.has(piece.type)){const edge=shape({...piece,type:'wall'},world.seed);if(world.structures.some(s=>DECK_GATE_EDGE_TYPES.has(s.type)&&(piece.type==='deckGate'||s.type==='deckGate')&&dist(shape({...s,type:'wall'},world.seed),edge)<.1))return 'This edge is occupied.';}
 ...
 const solid=['wall','doorway','perimeter','campGate','door','airlock','railing','deckGate'];
 ...
 if(['wall','doorway','perimeter','campGate','door','airlock','railing','deckGate','heater','cargo','workbench','lifeSupport','iceProcessor','garden','bed'].includes(piece.type))if(people.some(q=>{if(dist(q,b)>5)return false;const o=localOffset(b,q,b.frame),inside=Math.abs(o.x)<b.w/2+.5&&Math.abs(o.z)<b.d/2+.5;return inside&&(piece.type!=='doorway'||Math.abs(b.w>b.d?o.x:o.z)>.65);}))return 'A player is in the way.';
 if(piece.type==='deckGate'&&world.resources.some(n=>n.amount>0&&dist(n,b)<5&&Math.abs(localOffset(b,n,b.frame).x)<b.w/2+.15&&Math.abs(localOffset(b,n,b.frame).z)<b.d/2+.15))return 'Gather the resources on this edge first.';
