...
export function contextAction(world,p,{buildMode=false,reclaimMode=false,yaw=0}={}){
 ...
 if(target){const resource=target.type==='gather',object=resource?world.resources.find(n=>n.id===target.id):world.structures.find(s=>s.id===target.id);if(object)add(resource?object:shape(object,world.seed),resource?3:4,{kind:'message',message:target,label:resource?`${depositLabels[depositKind(object)]||'Gather'} ${RESOURCES[object.type].name}`:`${object.open?'Close':'Open'} ${RECIPES[object.type].name}`,repeat:resource});}
