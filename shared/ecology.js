import {direction,travel} from './planet.js';
import {rng,dist,blocked} from './world.js';
export const CYCLE=600;
export function daylight(time,settings){const daySeconds=settings?.daySeconds||360,cycle=daySeconds+(settings?.nightSeconds||240),raw=((time%cycle)+cycle)%cycle,night=raw>=daySeconds;return{night,phase:night?360+(raw-daySeconds)/(cycle-daySeconds)*240:raw/daySeconds*360,day:Math.floor(time/cycle)+1,remaining:Math.ceil((night?cycle:daySeconds)-raw)};}
...
export function activeCreature(c,time,settings){return c.health>0&&(c.type!=='prowler'||daylight(time,settings).night);}
export function clearPath(world,a,b){const distance=dist(a,b),n=Math.ceil(distance/.25),v=direction(a,b),length=Math.hypot(v.x,v.z)||1;if(n>1000)return false;for(let i=1;i<n;i++){const p=travel(a,v.x/length*distance*i/n,v.z/length*distance*i/n);if(blocked(world,p.x,p.z))return false;}return true;}
export function stepCreatures(world,players,dt){if(!players.length)return;for(const c of world.creatures){const spec=SPECIES[c.type];if(c.health<=0){if(world.time>=c.respawnAt&&!blocked(world,c.homeX,c.homeZ)){c.x=c.homeX;c.z=c.homeZ;c.health=spec.health;}else continue;}if(!activeCreature(c,world.time,world.settings))continue;
