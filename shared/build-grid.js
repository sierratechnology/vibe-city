...
export function snapBuild(world,target){if(planetDistance(target,{x:0,z:0})<70)return{x:Math.round(target.x/3)*3+0,z:Math.round(target.z/3)*3+0};const nearest=world.structures.filter(s=>s.site&&planetDistance(s,target)<100).sort((a,b)=>planetDistance(a,target)-planetDistance(b,target))[0],site=nearest?.site||tileAt(target.x,target.z),a=anchor(site),o=localOffset(a,target,a),factor=(RADIUS+planetHeight(a.x,a.z,world.seed))/RADIUS,gx=Math.round(o.x*factor/3),gz=Math.round(o.z*factor/3),position=gridPoint(site,gx,gz,world.seed);return{x:position.x,z:position.z,site,gx,gz};}
