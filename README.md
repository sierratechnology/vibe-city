...
A freestanding **Workbench** costs the same established material bundle as a cargo locker: 6 ferrite + 2 fiber. It now uses one original bounded authored fabrication-station mesh with a fail-closed primitive fallback; this is one visual tracer, not a complete art pass or broad asset pipeline. Within 3 m, its owner can use Field Guide → Fabrication to grant or revoke meal-queue access for individual saved collaborators. The owner and authorized collaborators spend their own 1 meat + 1 fiber and receive their own finished Field meals. The shared server-owned queue runs one five-second job with at most two pending and grants each meal once; each player may cancel only their own pending Field meal for an exact ingredient refund. When another pending meal follows theirs, a nearby player may also let that next meal go first, moving only their own pending meal one position later. The nearby builder may cancel the earliest collaborator-owned pending meal, including for an offline saved character; its ingredients return only to that original character. Active, blocked-output and owner-owned jobs cannot be cancelled through moderation. A full recipient backpack leaves the active job visibly blocked without losing value or allowing later jobs to bypass it. A grant does not confer ownership, dismantling, cargo, account, or administrative authority. Workbench time advances only while the server runs; there is no offline catch-up. Active queues must finish before dismantling. Broader roles, groups, organizations, automation, recipe expansion, arbitrary queue reordering, moving jobs earlier, bulk controls and offline invitations remain planned.
...
Backpacks hold **60 item units total**; equipped tools do not take space. **Cargo lockers hold 200 units**, cost 6 ferrite + 2 fiber, and can be placed on terrain. Approach within 3 m and choose Storage to deposit/withdraw a chosen quantity. A builder can set a PIN, then grant or revoke one saved collaborator's full deposit/withdraw access without sharing it; changing the PIN clears prior unlocks and collaborator grants. This does not grant ownership, dismantling, construction, Workbench, account or administrative authority. Lockers must be emptied before the builder dismantles them. Full inventories reject additional items; hunting and ruin rewards leave overflow on the ground.
