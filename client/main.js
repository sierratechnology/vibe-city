...
import {planetRenderer,vehicleRenderer,vector,orient,fromPoint} from './planet-renderer.js';
import {travel,basis,point,MONUMENTS} from '/shared/planet.js';
...
function buildEnvironment(seed){
 ...
 planet?.dispose();planet=planetRenderer(scene,seed);terrain=planet.terrain;planet.update(me()||{x:0,z:3});
 for(const type of Object.keys(RESOURCES)){const geo=type==='fiber'?new THREE.ConeGeometry(.65,1.9,5):type==='crystal'?new THREE.OctahedronGeometry(.85,0):new THREE.DodecahedronGeometry(.85,0);const m=mat(RESOURCES[type].color,type==='crystal'?{emissive:0x4b3d95,emissiveIntensity:.5}:{});const inst=new THREE.InstancedMesh(geo,m,1000);inst.castShadow=true;worldGroup.add(inst);resourceMeshes[type]=inst;}
...
function pieceMesh(piece,ghost=false){const group=new THREE.Group(),s=shape(piece,state.seed);group.position.copy(vector(s.x,s.z,s.y));orient(group,s.frame.x,s.frame.z);const color=ghost?new THREE.MeshBasicMaterial({color:0xb9f1c7,transparent:true,opacity:.48,depthWrite:false}):(materials[piece.type==='floor'?'deck':piece.type]||materials.wall);box(s.w,s.h,s.d,color,group);
...
function updateGeometry(){const rk=state.resources.map(n=>n.id+':'+n.amount).join(',');if(rk!==resourceKey){resourceKey=rk;for(const type of Object.keys(RESOURCES)){let i=0;for(const n of state.resources.filter(n=>n.type===type&&n.amount>0)){const size=.6+n.amount*.1;dummy.position.copy(vector(n.x,n.z,n.y+(type==='fiber'?.8:.6)));orient(dummy,n.x,n.z);dummy.rotateY(n.x*1.7);if(type==='crystal')dummy.rotateZ(.12);dummy.scale.setScalar(size);dummy.updateMatrix();resourceMeshes[type].setMatrixAt(i++,dummy.matrix);}resourceMeshes[type].count=i;resourceMeshes[type].instanceMatrix.needsUpdate=true;resourceMeshes[type].computeBoundingSphere();}}
 const sk=state.structures.map(s=>s.id+':'+!!s.open).join(',');if(sk!==structureKey){structureKey=sk;clearGroup(structuresGroup);for(const s of state.structures){if(s.open)continue;structuresGroup.add(pieceMesh(s));}}}
...
function interact(){const p=me();if(!p)return;const monument=MONUMENTS.find(m=>m.id!=='relay'&&dist(m,p)<8);if(monument){send({type:'salvage',id:monument.id});return;}if(dist(p,RUIN)<4)send({type:'scan'});else{const n=nearest();if(n)send({type:'gather',id:n.id});}}
function updateHUD(){const p=me();if(!p)return;$('network').textContent=`${state.playerCount||state.players.length} EXPLORER${state.playerCount===1?'':'S'} ONLINE`;
 const day=daylight(state.time,state.settings);$('dayClock').textContent=`DAY ${day.day} · ${day.night?'NIGHT':'DAYLIGHT'} · ${day.remaining}s`;const storm=state.time%150>=105;$('weather').textContent=storm?`ION WIND · ${Math.ceil(150-state.time%150)}s`:`CALM · WIND IN ${Math.ceil(105-state.time%150)}s`;
...
window.addEventListener('keydown',e=>{if(!joined||['INPUT','SELECT'].includes(document.activeElement.tagName))return;if(['Tab','Space','KeyW','KeyA','KeyS','KeyD'].includes(e.code))e.preventDefault();if(e.repeat)return;
 ...
 if(e.code==='KeyB'){buildMode=!buildMode;updateHUD();}if(/^Digit[0-9]$/.test(e.code)&&!buildMode){terminal.useBelt((Number(e.code.slice(-1))+9)%10);return;}if(/^Digit[1-7]$/.test(e.code)){selected=pieces[+e.code.slice(-1)-1];buildMode=true;updateHUD();}if(e.code==='KeyR')rotation=(rotation+1)%4;if(e.code==='KeyE'){interact();lastGather=performance.now();}
...
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.05,(now-last)/1000);last=now;frameCount++;if(now-fpsTime>1000){fps=Math.round(frameCount*1000/(now-fpsTime));frameCount=0;fpsTime=now;}
 ...
 const sprint=!!(keys.ShiftLeft||keys.ShiftRight||controls.input.sprint),speed=sprint&&p.stamina>5&&p.charge>5?6.5:4.2;
 if(!p.vehicle&&!p.sleeping){const next=travel(localPos,mx*dt*speed,mz*dt*speed);if(!blocked(state,next.x,next.z)){localPos.x=next.x;localPos.z=next.z;}}else{localPos.x=p.x;localPos.z=p.z;}
 ...
 const nextBasis=basis(localPos.x,localPos.z);if(beforeMove.distanceTo(localPos)>.001)yaw=Math.atan2(oldArm.x*nextBasis.east.x+oldArm.y*nextBasis.east.y+oldArm.z*nextBasis.east.z,oldArm.x*nextBasis.south.x+oldArm.y*nextBasis.south.y+oldArm.z*nextBasis.south.z);localPos.y=surface(state,localPos.x,localPos.z);if(now-lastInput>50){sendInput({x:mx,z:mz,vx:inputBasis.east.x*mx+inputBasis.south.x*mz,vy:inputBasis.east.y*mx+inputBasis.south.y*mz,vz:inputBasis.east.z*mx+inputBasis.south.z*mz,sprint,yaw,aimYaw:yaw+Math.PI});lastInput=now;}
 ...
 for(const player of state.players){let a=avatars.get(player.id);if(!a){a=astronaut(player.id===id?0xb6d4c2:0xebac87);if(player.id!==id){const tag=document.createElement('canvas');tag.width=256;tag.height=64;const ctx=tag.getContext('2d');ctx.fillStyle='#132a31d9';ctx.fillRect(0,0,256,64);ctx.fillStyle='#e6f7e8';ctx.font='bold 26px sans-serif';ctx.textAlign='center';ctx.fillText(player.name,128,41,240);const label=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(tag),depthTest:false}));label.position.y=2.4;label.scale.set(2.3,.575,1);a.add(label);}scene.add(a);avatars.set(player.id,a);a.position.copy(vector(player.x,player.z,surface(state,player.x,player.z)));}
 const target=player.id===id?vector(localPos.x,localPos.z,localPos.y):vector(player.x,player.z,surface(state,player.x,player.z));const moving=a.position.distanceTo(target)>.03;a.position.lerp(target,player.id===id?1:Math.min(1,dt*12));orient(a,player.x,player.z);a.rotateY(player.yaw);a.visible=!player.vehicle;for(let i=0;i<a.userData.legs.length;i++)a.userData.legs[i].rotation.x=moving?Math.sin(now*.012+i*Math.PI)*.4:0;
 }
 if(ruinPulse){ruinPulse.rotation.y=now*.001;ruinPulse.position.y=1.7+Math.sin(now*.002)*.15;}
 const phase=daylight(state.time,state.settings),sunFade=phase.phase<360?Math.min(1,phase.phase/25,(360-phase.phase)/25):0;skyLight.intensity=.22+sunFade*1.58;sun.intensity=.06+sunFade*2.44;scene.background.set('#0c142b').lerp(new THREE.Color('#718f96'),sunFade);const orbit=phase.phase/360*Math.PI;const centre=joined?vector(localPos.x,localPos.z,localPos.y):new THREE.Vector3();const lightBasis=basis(localPos.x,localPos.z);skyLight.position.set(lightBasis.up.x,lightBasis.up.y,lightBasis.up.z);sun.position.copy(centre).addScaledVector(new THREE.Vector3(lightBasis.east.x,lightBasis.east.y,lightBasis.east.z),-Math.cos(orbit)*40).addScaledVector(new THREE.Vector3(lightBasis.up.x,lightBasis.up.y,lightBasis.up.z),Math.max(3,Math.sin(orbit)*48));sun.target.position.copy(centre);sun.target.updateMatrixWorld();scene.fog.color.copy(scene.background);wildlife.update(state,dt,id,yaw,shape);vehicles.update(state);
...
const terminal=expeditionUI({get:()=>({state,player:me()}),send,pause:open=>{guide=open;keys={};controls.reset();moveTarget=null;sendInput({x:0,z:0});},notify,attack});
