...
import {height,surface,generate,rng,RUIN,RECIPES,RESOURCES,shape,dist,placementError,canAfford,sheltered,powered,blocked} from '/shared/world.js';
...
scene.add(new THREE.HemisphereLight(0xc4e5e0,0x573746,1.8));const sun=new THREE.DirectionalLight(0xffd4a6,2.5);sun.position.set(-24,48,18);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-45,right:45,top:45,bottom:-45,near:1,far:110});sun.shadow.bias=-.001;scene.add(sun);
const mat=(color,opts={})=>new THREE.MeshStandardMaterial({color,roughness:.85,flatShading:true,...opts});
const materials={deck:mat(0x4d666c),wall:mat(0xd3d0b7),roof:mat(0x8fa99a),heater:mat(0x384e59),trim:mat(0xf4b28f),glow:mat(0x9effd5,{emissive:0x57bb9e,emissiveIntensity:1}),dark:mat(0x203c48)};
...
function updateGeometry(){const rk=state.resources.map(n=>n.amount).join(',');if(rk!==resourceKey){resourceKey=rk;for(const type of Object.keys(RESOURCES)){let i=0;for(const n of state.resources.filter(n=>n.type===type&&n.amount>0)){const size=.6+n.amount*.1;dummy.position.set(n.x,n.y+(type==='fiber'?.8:.6),n.z);dummy.rotation.set(0,n.x*1.7,type==='crystal'?.12:0);dummy.scale.setScalar(size);dummy.updateMatrix();resourceMeshes[type].setMatrixAt(i++,dummy.matrix);}resourceMeshes[type].count=i;resourceMeshes[type].instanceMatrix.needsUpdate=true;resourceMeshes[type].computeBoundingSphere();}}
...
const avatars=new Map();let localPos=new THREE.Vector3(0,0,3),yaw=0,pitch=.42,distance=8,keys={},buildMode=false,selected='floor',rotation=0,piece=null,lastInput=0,lastGather=0,guide=false,toastTimer;
...
function connectExplorer(){$('enter').disabled=true;$('joinError').textContent='Connecting to the expedition server…';
 const slot=$('slot').value;let token=localStorage.getItem(`vc-pilot-${slot}`);if(!token){token=crypto.randomUUID?.()||'pilot-'+Array.from(crypto.getRandomValues(new Uint32Array(4))).join('-');localStorage.setItem(`vc-pilot-${slot}`,token);}localStorage.setItem(`vc-name-${slot}`,$('name').value);
 ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/api/ws`);
 ws.onopen=()=>ws.send(JSON.stringify({type:'join',token,name:$('name').value}));
...
$('slot').onchange=()=>{$('name').value=localStorage.getItem(`vc-name-${$('slot').value}`)||`Explorer ${$('slot').value}`;};$('name').value=localStorage.getItem('vc-name-1')||'Explorer';
function renderRecipes(){const p=me();if(!p)return;$('backpack').textContent=Object.entries(p.inventory).map(([k,v])=>`${RESOURCES[k].name}: ${v}`).join(' · ');$('recipes').replaceChildren();for(const [key,r] of Object.entries(RECIPES)){const div=document.createElement('div');div.className='recipe';const button=document.createElement('button');button.textContent=key==='cutter'?(p.cutter?'EQUIPPED':'CRAFT'):r.unlock&&!p.unlocked?'LOCKED':'SELECT';button.disabled=(key==='cutter'&&(p.cutter||!canAfford(p,key)))||(r.unlock&&!p.unlocked);button.onclick=()=>{if(key==='cutter')send({type:'craft',recipe:key});else{selected=key;buildMode=true;guideOpen(false);updateHUD();}};const title=document.createElement('b');title.textContent=r.name;const cost=document.createElement('small');cost.textContent=Object.entries(r.cost).map(([k,v])=>`${v} ${RESOURCES[k].name}`).join(' + ');const desc=document.createElement('p');desc.textContent=r.description;div.append(button,title,cost,desc);$('recipes').append(div);}}
const pieces=['floor','wall','roof','heater'];pieces.forEach((key,i)=>{const b=document.createElement('button');b.innerHTML=`<b>${i+1}</b>${RECIPES[key].name}<small>${Object.entries(RECIPES[key].cost).map(([k,v])=>v+' '+k).join(' / ')}</small>`;b.onclick=()=>{selected=key;buildMode=true;updateHUD();};b.dataset.piece=key;$('hotbar').append(b);});
...
function updateHUD(){const p=me();if(!p)return;$('network').textContent=`${state.players.length} EXPLORER${state.players.length===1?'':'S'} ONLINE`;
 const storm=state.time%150>=105;$('weather').textContent=storm?`ION WIND · ${Math.ceil(150-state.time%150)}s`:`CALM · WIND IN ${Math.ceil(105-state.time%150)}s`;
 ...
 $('inventory').textContent=`${p.inventory.ferrite} Ferrite   ·   ${p.inventory.fiber} Fiber   ·   ${p.inventory.crystal} Flux`;
 ...
 $('placeAction').disabled=!buildMode;$('rotateAction').disabled=!buildMode;$('buildAction').textContent=buildMode?'Exit build':'Build';$('buildHint').classList.toggle('hidden',!buildMode);const n=nearest();$('interaction').textContent=buildMode?'':dist(p,RUIN)<4?'[E] Read signal console':n?`[E] Gather ${RESOURCES[n.type].name} · ${n.amount} remaining`:document.pointerLockElement===canvas?'E gather nearby · C field guide':'Tap terrain to walk · Drag to look';
}
window.addEventListener('keydown',e=>{if(!joined||['INPUT','SELECT'].includes(document.activeElement.tagName))return;if(['Tab','Space','KeyW','KeyA','KeyS','KeyD'].includes(e.code))e.preventDefault();if(e.repeat)return;
 if(e.code==='KeyC'){guideOpen(!guide);return;}if(e.code==='Escape'){if(guide)guideOpen(false);else if(buildMode){buildMode=false;updateHUD();}return;}if(guide)return;keys[e.code]=true;
 if(e.code==='KeyB'){buildMode=!buildMode;updateHUD();}if(/^Digit[1-4]$/.test(e.code)){selected=pieces[+e.code.slice(-1)-1];buildMode=true;updateHUD();}if(e.code==='KeyR')rotation=(rotation+1)%4;if(e.code==='KeyE'){interact();lastGather=performance.now();}
...
const controls=bindControls({canvas,
 orbit:(dx,dy)=>{if(!joined||guide)return;yaw-=dx*.003;pitch=Math.max(.15,Math.min(1.1,pitch+dy*.003));},
 ...
 zoom:step=>distance=Math.max(4,Math.min(13,distance+step))
});
canvas.addEventListener('wheel',e=>{distance=Math.max(4,Math.min(13,distance+e.deltaY*.01));e.preventDefault();},{passive:false});
...
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.05,(now-last)/1000);last=now;frameCount++;if(now-fpsTime>1000){fps=Math.round(frameCount*1000/(now-fpsTime));frameCount=0;fpsTime=now;}
 ...
 localPos.y=surface(state,localPos.x,localPos.z);if(now-lastInput>50){send({type:'input',x:mx,z:mz,sprint,yaw});lastInput=now;}
 if((keys.KeyE||controls.input.gather)&&!guide&&now-lastGather>(p.cutter?420:800)){interact();lastGather=now;}
 camera.position.set(localPos.x+Math.sin(yaw)*distance*Math.cos(pitch),localPos.y+1.5+distance*Math.sin(pitch),localPos.z+Math.cos(yaw)*distance*Math.cos(pitch));const focus=new THREE.Vector3(localPos.x,localPos.y+1.15,localPos.z);const arm=camera.position.clone().sub(focus);cameraRay.set(focus,arm.clone().normalize());cameraRay.far=arm.length();const obstruction=cameraRay.intersectObjects(structuresGroup.children,true).find(h=>h.distance>.15&&h.object.geometry?.type==='BoxGeometry');if(obstruction)camera.position.copy(focus).addScaledVector(arm.normalize(),Math.max(.4,obstruction.distance-.25));camera.lookAt(focus);
