...
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.05,(now-last)/1000);last=now;frameCount++;if(now-fpsTime>1000){fps=Math.round(frameCount*1000/(now-fpsTime));frameCount=0;fpsTime=now;}
 ...
 const nextBasis=basis(localPos.x,localPos.z);if(beforeMove.distanceTo(localPos)>.001)yaw=Math.atan2(oldArm.x*nextBasis.east.x+oldArm.y*nextBasis.east.y+oldArm.z*nextBasis.east.z,oldArm.x*nextBasis.south.x+oldArm.y*nextBasis.south.y+oldArm.z*nextBasis.south.z);localPos.y=surface(state,localPos.x,localPos.z);if(now-lastInput>50){send({type:'input',x:mx,z:mz,vx:inputBasis.east.x*mx+inputBasis.south.x*mz,vy:inputBasis.east.y*mx+inputBasis.south.y*mz,vz:inputBasis.east.z*mx+inputBasis.south.z*mz,sprint,yaw,aimYaw:yaw+Math.PI});lastInput=now;}
...
const terminal=expeditionUI({get:()=>({state,player:me()}),send,pause:open=>{guide=open;keys={};controls.reset();moveTarget=null;send({type:'input',x:0,z:0});},notify,attack});
