...
try{
 ...
 await activate(collaborator,collaborator.page.locator('#menuButton'));const collaboratorMeal=collaborator.page.locator('.recipe').filter({hasText:'Field meal'}).getByRole('button');assert.equal(await collaboratorMeal.isEnabled(),true);await activate(collaborator,collaboratorMeal);await collaborator.page.waitForFunction(()=>window.vibeDiagnostics.state.structures.find(s=>s.id==='browser-bench')?.water===1);
 await activate(stranger,stranger.page.locator('#menuButton'));const strangerMeal=stranger.page.locator('.recipe').filter({hasText:'Field meal'}).getByRole('button');assert.equal(await strangerMeal.isDisabled(),true);assert.match(await stranger.page.locator('.recipe').filter({hasText:'Field meal'}).innerText(),/Move within 3 m of a workbench you may use/);
