...
try{
 ...
 const meal=page.locator('.recipe').filter({has:page.locator('b',{hasText:'Field meal'})});await meal.locator('button:not(.pin-recipe)').waitFor({state:'visible'});assert.equal(await meal.locator('button:not(.pin-recipe)').getAttribute('aria-label'),'Queue field meal');assert.match(await meal.locator('p').innerText(),/Ready · 5 server seconds/);
 if(engine==='chromium'&&mode==='desktop'){await meal.locator('button:not(.pin-recipe)').focus();assert.equal(await page.evaluate(()=>document.activeElement?.getAttribute('aria-label')),'Queue field meal');await page.keyboard.press('Enter');await page.waitForFunction(()=>window.vibeDiagnostics.state.structures.find(s=>s.id==='browser-bench')?.water===1);await new Promise(resolve=>setTimeout(resolve,700));assert.equal(await page.evaluate(()=>document.activeElement?.getAttribute('aria-label')),'Queue field meal');}else{await activate(meal.locator('button:not(.pin-recipe)'));await page.waitForFunction(()=>window.vibeDiagnostics.state.structures.find(s=>s.id==='browser-bench')?.water===1);}await page.waitForFunction(()=>document.querySelector('.recipe p')&&[...document.querySelectorAll('.recipe')].some(r=>r.textContent.includes('Field meal')&&r.textContent.includes('IN PROGRESS')));
