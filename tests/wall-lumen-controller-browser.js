...
const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'vibe-wall-lumen-controller-'));
...
try {
  ...
  const account = await apiAccount(base, 94);
  ...
  await page.addInitScript(() => {
    window.testPad = {id: 'Wall lumen virtual standard controller', index: 0, connected: true, mapping: 'standard', axes: [0, 0, 0, 0], buttons: Array.from({length: 16}, () => ({pressed: false})), samples: 0};
  ...
  app.game.world.structures.push(
    {id: 'wall', type: 'wall', x: 9, z: 0, rotation: 0, owner: id},
    {id: 'lamp', type: 'lamp', x: 9, z: 0, rotation: 0, owner: id, on: true},
  );
  app.save();
  await page.waitForFunction(() => document.querySelector('#interaction').textContent.includes('Wall lumen on · Turn off Wall lumen'));

  await press(7);
  await page.waitForFunction(() => window.vibeDiagnostics.state.structures.find(s => s.id === 'lamp')?.on === false);
  assert.equal(app.game.world.structures.find(s => s.id === 'lamp').on, false);
  ...
  await press(0);
  await page.waitForFunction(() => document.querySelector('#gatherAction').textContent === 'Reclaim Wall lumen');
  await press(7);
  await page.waitForFunction(() => !window.vibeDiagnostics.state.structures.some(s => s.id === 'lamp'));
  assert.deepEqual({ferrite: player.inventory.ferrite, crystal: player.inventory.crystal}, {ferrite: 1, crystal: 1});
  assert.deepEqual(errors, []);
  assert.deepEqual(external, []);
  console.log('virtual standard controller: Wall lumen toggle, reclaim targeting, and exact refund passed; physical controller unverified');
  await context.close();
} finally {
