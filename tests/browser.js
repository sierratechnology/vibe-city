...
try{
 ...
 await gather(a,1);await a.keyboard.press('KeyC');await a.locator('#recipes button').first().click();await a.waitForFunction(()=>window.vibeDiagnostics.player.cutter);await a.locator('#closeGuide').click();
 ...
 await a.keyboard.press(key);await pause(250);let d=await diag(a);assert.equal(d.preview.x,9);assert.equal(d.preview.z,0);
 ...
 fs.mkdirSync('docs/screenshots',{recursive:true});await a.screenshot({path:'docs/screenshots/outpost.png'});await b.screenshot({path:'docs/screenshots/second-player.png'});
 ...
 const report={at:new Date().toISOString(),passed:true,browsers:2,renderer:'Chrome headless / Metal requested',checks:['two visible avatars','keyboard movement','remote movement','shared resource depletion','gather/craft/scan loop','four modular pieces via preview and clicks','shared construction','completion','server restart persistence'],renderSample:perf,pageErrors:errors};fs.writeFileSync('docs/browser-test-report.json',JSON.stringify(report,null,2));
